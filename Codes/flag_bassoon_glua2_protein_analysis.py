# -*- coding: utf-8 -*-
"""Step 1) Data_into_range_Flag_Bassoon_GluA2.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1oTpr9W1rb1wqyXMwS_pD--QVl-SmPPOd
...
id_name = "G93L_s1h2_SR_2_"
factor = 4.2088
folder_path = "Data"

#enrichment analysis variables
proteins = ['Flag-Bassoon', 'Flag-GluA2']
enrichment_analysis = True

#angle analysis is not used in this set
...
for n in range(0,length):
  ...
  if enrichment_analysis == True:
    ...
    all_bass.append(df_1_all)
    all_glua2.append(df_3_all)
    all_bass_complex.append(df_1_flag_bass_complex)
  ...
  #goes through the df_1_loc (Bassoon and Flag) and df_2_loc (Bassoon to GluA2), finds the same Flag marker by SpotId and then compares the distances, both must be under 300nm
  new_data = []
  for index1, row1 in df_1_loc.iterrows():
    if row1['Actual_Min_Distance'] < 0.300:
        matching_rows = df_2_loc[
            ...
            #Bassoon to Flag distance must be smaller than Bassoon to GluA2 distance
            (df_2_loc['Actual_Min_Distance'] > row1['Actual_Min_Distance'])
        ]
  ...
  #goes though df_3_loc and removes any markers by spot index that aren't in new_df, so this removes any Flag2x-GluA2 markers that aren't near a Bassoon
  new_data_2 = []
  ...
  for index, row in new_df.iterrows():
    ...
    #Find matching rows in 'df_3_loc'
    matching_rows_3 = df_3_loc[
    #Flag equals to Flag Index
    (df_3_loc['SpotIndex1'] == row['ClosestSpotIndex_1']) &
    #GluA2 equals to GluA2 Index
    (df_3_loc['ClosestSpotIndex2'] == row['ClosestSpotIndex_2']) &
    #Bassoon to Flag + Flag to GLuA2 must be equal to Bassoon to GluA2 with a given 10% buffer range for Basssoon to GluA2
  ...
  filtered_data = []
  #this removes any Flag to GluA1 or Flug to GluA2 that don't have GluA1 and GluA2 within 60nm
  #This goes through the rows of new_df, it compares it to df_second, it removes any rows where the GluA1 isn't also found in df_second
  i = 0
  j = 0
  while i < len(new_df) and j < len(df_second):
    #Flag equals to Flag Index and GluA2 equals to GluA2
    if new_df['ClosestSpotIndex_1'].iloc[i] == df_second['SpotIndex'].iloc[j] and new_df['ClosestSpotIndex_2'].iloc[i] == df_second['ClosestSpotIndex_3_1'].iloc[j]:
  ...
  merged_df = pd.merge(new_df, df_second, left_on=['ClosestSpotIndex_1', 'ClosestSpotIndex_2'],
  ...
  if use_angle_analysis == True:
    ...
    #angle of flag
    merged_df['Angle_of_Bass'] = np.arccos((a**2 + b**2 - c**2) / (2 * a * b))
    merged_df['Angle_of_Bass'] = merged_df['Angle_of_Bass'].apply(math.degrees)
    ...
    merged_df = merged_df[(merged_df['Angle_of_Flag'] < angle_flag) & (merged_df['Angle_of_GluA2'] < angle_glua2) & (merged_df['Angle_of_Bass'] < angle_Bass) & (merged_df['Angle_of_Flag'] > angle_flag_lower)]
...
if enrichment_analysis == True:
  ...
  data = {
    ...
    'Total Interactions': [total_all_flag_bass, total_all_flag_glua2],
    'Close Interactions': [total_close_flag_bass, total_close_flag_glua2]
  }
  ...
  merged_bass_all = pd.concat(all_bass, ignore_index=True)
  merged_glua2_complex = pd.concat(all_glua2_complex, ignore_index=True)
  merged_bass_complex = pd.concat(all_bass_complex, ignore_index=True)
  ...
  with pd.ExcelWriter(f'{id_name}_raw_enrichment_data.xlsx') as writer:
      ...
      merged_bass_complex.to_excel(writer, sheet_name='Flag_Bassoon_Complex', index=False)
